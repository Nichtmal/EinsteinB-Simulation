...
def run_simulation(element, ionized=False, num_states=10, grid_spacing=0.1, dimension = 15, conv_energy = 1e-4):
    """
    ...
    # Define the atom
    adjusted_dimension = round(dimension / grid_spacing) * grid_spacing # Adjusting the dimension to ensure a cubic grid
    print(f"New dimensions: {adjusted_dimension}, with {adjusted_dimension / grid_spacing} points per dimension")
    atom = Atoms(atom_label, positions=[(0, 0, 0)], cell=[adjusted_dimension, adjusted_dimension, adjusted_dimension], pbc=False)
    ...
    # Collect results
    results = {'dimension': adjusted_dimension, 'resolution': grid_spacing, 'convergence': conv_energy, 'states': []}
