...
# database_file = 'C:\Users\Tiago\Kreuzgasse Onedrive\Desktop\Tiago\Corona Aufgaben\Physik\GYPT Theorie\Database\Simulation_database.h5'
...
def sim_to_db(data, database_file):
    ...
    with h5py.File(database_file, 'a') as db:
        ...
        set_name = set_name.replace('.', '_')  # Avoid issues with special characters in group names
        ...
        if set_name not in sim_group:
            ...
            current_set.create_dataset('eigenvalues', shape=(0,), maxshape=(None,), dtype='f8')
            current_set.create_dataset('wavefunctions', shape=(0,), maxshape=(None,), dtype=h5py.vlen_dtype(np.float64))
            current_set.create_dataset('number', shape=(0,), maxshape=(None,), dtype='i4')
            current_set.create_dataset('spins', shape=(0,), maxshape=(None,), dtype='i4')
            dt_atom = h5py.string_dtype(encoding='utf-8')
            current_set.create_dataset('atoms', shape=(0,), maxshape=(None,), dtype=dt_atom)
        else:
        ...
        # Resize datasets to accommodate new data
        current_set['eigenvalues'].resize(int(num_existing_states + num_new_states), axis=0)
        current_set['wavefunctions'].resize(int(num_existing_states + num_new_states), axis=0)
        ...
        for i, state in enumerate(results['states']):
            ...
            current_set['wavefunctions'][num_existing_states + i] = state['wavefunction'].flatten() # Use np.flatten() for storing function as 1D array
...
def corr_to_db(desired_states, Z_eff, Bs, set_name, Atom, database_file):
    ...
    with h5py.File(database_file, 'r+') as db:  # Open in read/write mode
        ...
        data = {
            ...
            'states': [
                {'number': numbers[i], 'eigenvalue': eigenvalues[i], 'wavefunction': wavefunctions[i], 'spin': spins[i], 'atom': Atom}
                for i in range(len(numbers))
            ]
        }
        length = int(data['dimension'] / data['resolution']) if int(data['dimension'] / data['resolution']) == data['dimension'] / data['resolution'] else -9
        if length == -9:
            raise TypeError
        ...
        if corr_group_name not in db['simulations']:
            ...
            corr_group.create_dataset('Bs', shape=(0,), maxshape=(None,), dtype=h5py.special_dtype(vlen=np.dtype('float64')))
            corr_group.create_dataset('wavefunctions', shape=(0, n, m), maxshape=(None, n, m), dtype='complex128')
            corr_group.create_dataset('base_energy', shape=(0,), maxshape=(None,), dtype='float64')
        ...
        for state in results['states']:
            ...
            corr_group['wavefunctions'].resize((corr_group['wavefunctions'].shape[0] + 1, n, m))
    ...
    for atom in atoms:
        ...
        with h5py.File(database_file, 'r') as db:
            ...
            for i in range(states_len):
                print(dataset['n'][i], dataset['l'][i], dataset['j'][i], dataset['m_j'][i], dataset['j'][i] - dataset['l'][i])
            ...
            for i in range(states_len):
                numbers = (dataset['n'][i], dataset['l'][i], dataset['j'][i], dataset['m_j'][i], dataset['j'][i] - dataset['l'][i])
                if dataset['n'][i] != 0 and numbers not in done: # Disregarding non-physical states with quantum numbers 0
                    done.append(numbers)
                    state_data = {
                            'n': dataset['n'][i],
                            'l': dataset['l'][i],
                            ...
                            'wavefunctions': dataset['wavefunctions'][i-a], # adjusting indices
                            'base_energy': dataset['base_energy'][i-a], # Will be modified by B in other scripts
                            'atom': atom
                            }
                    # print(state_data)
                    if not any(
                        d['n'] == state_data['n'] and
                        d['l'] == state_data['l'] and
                        d['j'] == state_data['j'] and
                        d['s'] == state_data['s'] and
                        d['m_j'] == state_data['m_j'] and
                        d['atom'] == state_data['atom'] and
                        np.array_equal(d['wavefunctions'], state_data['wavefunctions']) and
                        np.array_equal(d['Bs'], state_data['Bs']) and
                        d['base_energy'] == state_data['base_energy']
                        for d in states
                    ):
...
def einstein_to_db(data, set_name, database_file):
    ...
    with h5py.File(database_file, 'a') as db:
        if set_name not in db:
            ...
            # Initialize datasets
            table_group.create_dataset('initial_state', shape=(0,), maxshape=(None,), dtype=h5py.vlen_dtype(str))
            table_group.create_dataset('resulting_state', shape=(0,), maxshape=(None,), dtype=h5py.vlen_dtype(str))
        ...
        for i, state in enumerate(data['states']):
            ...
            table_group['resulting_state'][current_size + i] = str(state['resulting_state'])
...
def import_einstein_from_db(set_name, database_file):
    ...
    with h5py.File(database_file, 'r') as db:
        ...
        data = {
            'initial_states': [state.decode('utf-8') for state in table_group['initial_state']],
            'final_states': [state.decode('utf-8') for state in table_group['resulting_state']],
