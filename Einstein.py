...
# Calculating the absorbtion coefficient as a function of the frequency
def abs_coeff(B12, delta_E, T, f_nu, nu):
    dE = abs(delta_E)
    beta = k * T
    return n_1 * B12 * hbar/2 * (1-np.exp(-dE * beta)) * nu * f_nu

# Setting up Voigt profile
...
def is_transition_allowed(state1, state2):
    ...
    n1, l1, s1, j1, mj1 = state1["n"], state1["l"], state1["s"], state1["j"], state1["m_j"]
    n2, l2, s2, j2, mj2 = state2["n"], state2["l"], state2["s"], state2["j"], state2["m_j"]
...
def g_j(j, l, s):
    return (1 + (j*(j+1) - l*(l-1) + s*(s+1))/(2*j*(j+1)) if j != 0 else 0)

def h_j(j, l, n):
    return (j*(j+1) - l*(l+1)-3/4)/(l*(l+1)*(l+1/2)) / (4 * n**3)
...
def delta_Ef(state_attr1, state_attr2, B, Z):
    ...
    return E2 - E1 + (hbar * gamma * B * (g_j1 * m_j1 - g_j2 * m_j2))/e + factor_SOC * (h_j2 - h_j1)/e # In eV
...
def calculate_ESC(data, Z):
    ...
    results = {'resolution': gridsize, 'B_range': data['B_range'], 'B_res': data['B_res'], 'states': []}
    ...
    for state in data['states']:
        ...
        wave = state['wavefunctions'][0] / np.sqrt((np.sum(np.abs(state['wavefunctions'][0])**2)*dx**3))
        print(f"Test integral (has to be 1): {integral_f(wave, wave, 1, dx)}")
        
        wave = state['wavefunctions'][0] / np.sqrt(integral_f(state['wavefunctions'][0], state['wavefunctions'][0], 1, dx))
        print(f"Test integral (has to be 1): {integral_f(wave, wave, 1, dx)}")
        ...
        for i in range(len(state['Bs'])):
            ...
            for state2 in data['states']:
                state2_attr = (state2['n'], state2['l'], state2['j'], state2['m_j'], state2['base_energy'])
                ...
                if is_transition_allowed(state1, state2):
                    ...
                    if abs(delta_E) > 0:
                        ...
                        # print(R['initial_state'], R['resulting_state'])
